...
def btc_command(update, context):
    update.message.reply_text("💰 Giá BTC hiện tại: ...")  # Cập nhật giá thực tế

def aud_command(update, context):
    update.message.reply_text("💵 Giá AUD hiện tại: ...")  # Cập nhật giá thực tế

def main():
    token = "YOUR_BOT_TOKEN"  # Thay bằng token thật
    updater = Updater(token, use_context=True)
    dp = updater.dispatcher  # Lấy dispatcher

    # Đăng ký lệnh
    dp.add_handler(CommandHandler("start", start))
    dp.add_handler(CommandHandler("btc", btc_command))
    dp.add_handler(CommandHandler("aud", aud_command))
